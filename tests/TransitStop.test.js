...
describe('TransitStop grouping', () => {
  it('groups bus arrivals by direction then route', () => {
    ...
    expect(grouped.directions.map((direction) => direction.direction).sort()).toEqual([
      'Northbound',
      'Southbound'
    ]);

    const northbound = grouped.directions.find((direction) => direction.direction === 'Northbound');
  ...
  it('groups train arrivals by route then direction', () => {
    ...
    const destinationLabels = grouped.routes[0].destinations.map((destination) => destination.direction);
